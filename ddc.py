...
class DDC:
    """
    Class for a dynamic decision circuit (DDC).
    ...
    def __init__(self, filename):
        ...
        self._state_vars = DDC._get_state_vars(prog)
    ...
    def _add_utility_parameters(self, parsed_prog: ClauseDB) -> None:
        ...
        while state:
            ...
            parsed_prog.add_clause(Clause(utility_term, DDC.big_and(self._wrap_in_next_state_functor(state))))
            utility_idx += 1
            state = DDC.enumeration_next(state)
    ...
    def _init_utility(self) -> Dict[int, pn_weight]:
        ...
        for r in self._rewards:
            ...
            if abs(key) in labels:   # update the label
                DDC._update_utility_label(labels, key, self._rewards[r].compute_value())
    ...
    def _update_utility(self, labels: Dict[int, pn_weight]) -> float:
        ...
        for u in self._utilities:
            ...
            DDC._update_utility_label(labels, key, self._discount * updated_utilities[u])
