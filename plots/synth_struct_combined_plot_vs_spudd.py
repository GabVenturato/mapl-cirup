...
def create_plot(out_filename=None):
    ...
    assert len(x) > 0
    axs[0].plot(x, y, color=color_mapl_approx_vi, marker=".", label=label_mapl_approx_vi)
    ...
    assert len(x) > 0
    axs[0].plot(x, y, color=color_mapl_approx_kcvi, marker=".", label=label_mapl_approx_kcvi)
